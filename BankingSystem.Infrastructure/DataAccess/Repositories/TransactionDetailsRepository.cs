        ...
        public async Task<Dictionary<string, decimal>> GetTotalIncomeAsync(DateTime fromDate, DateTime toDate, string email)
        {
            ...
            if (_connection != null && _transaction != null)
            {
                var sql = @"SELECT ct.[Type] AS Currency, ISNULL(SUM(td.Amount),0) AS TotalIncome
                    FROM CurrencyType ct 
					LEFT JOIN TransactionDetails td ON ct.Id = td.CurrencyId
					    AND td.IsATM = 0 AND td.FromAccountId != td.ToAccountId 	
                        AND td.PerformedAt >= @fromDate AND td.PerformedAt <= @toDate
                    LEFT JOIN Account a ON a.Id = td.ToAccountId  
        ...
        public async Task<Dictionary<string, decimal>> GetTotalExpenseAsync(DateTime fromDate, DateTime toDate, string email)
        {
            ...
            if (_connection != null && _transaction != null)
            {
                var sql = @"SELECT ct.[Type], ISNULL(SUM(td.BankProfit + td.Amount),0) 
                              FROM CurrencyType AS ct 
                              LEFT JOIN TransactionDetails AS td  ON ct.Id = td.CurrencyId 
							    AND td.FromAccountId != td.ToAccountId 
                                AND td.PerformedAt >= '2025-01-01' AND td.PerformedAt <= '2025-02-25'
                              LEFT JOIN Account AS a ON td.FromAccountId = a.Id
                              LEFT JOIN Person AS p ON p.Id = a.PersonId                               
                                AND p.Email = 't@gmail.com' 
                              GROUP BY ct.[Type]";
                var sqlResult = await _connection.QueryAsync<(string, decimal)>(sql,
                    new { email, fromDate, toDate }, transaction: _transaction);
                result = sqlResult.ToDictionary(row => row.Item1, row => row.Item2);
